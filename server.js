...
app.get("/analyze/:sessionId", async (req, res) => {
  ...
  try {
    ...
    const total = Object.values(overallAnalysis.emotions).reduce((a, b) => a + b, 0);

    if (total > 0) {
      for (let emotion in overallAnalysis.emotions) {
        overallAnalysis.emotions[emotion] = ((overallAnalysis.emotions[emotion] / total) * 100).toFixed(2);
      }
    } else {
      for (let emotion in overallAnalysis.emotions) {
